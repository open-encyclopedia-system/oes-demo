            ...
            if ($field['key'] == 'field_demo_glossary__glossary_article'){
                ...
                if(!empty($field['value']))
                    foreach($field['value'] as $article){
                        ...
                        $language = (oes_get_post_language_label(get_parent_id($article)) ?: false);
                        if($language) $metaString .= $language;

                        /* get version information */
                        $version = get_version_field($article) ?: false;
                        if($version) $metaString .= empty($metaString) ? $version : ', Version ' . $version;

                        /* add meta information to archive list */
                        $title = oes_get_display_title($article);
                        $articles[$title . $article] = sprintf('<a href="%s">%s</a>%s',
